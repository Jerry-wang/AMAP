...
public class myTree {
   ...
   public DataLayerInterface dataLayer;
   
   //Tree Constructor
   public myTree(DataLayerInterface data, int[] treeRoot) throws  TreeTooBigException {
      
      dataLayer = data;
      
      CPlus = new PNMatrix(data.getTokenClasses().getFirst().getForwardsIncidenceMatrix(
    		  data.getArcsArrayList(), data.getTransitionsArrayList(), data.getPlacesArrayList()));
      CMinus = new PNMatrix(data.getTokenClasses().getFirst().getBackwardsIncidenceMatrix(
    		  data.getArcsArrayList(), data.getTransitionsArrayList(), data.getPlacesArrayList()));
      inhibition = new PNMatrix(data.getTokenClasses().getFirst().getInhibitionMatrix(
   ...
   //Tree Constructor
   public myTree(DataLayerInterface data, int[] treeRoot, File reachabilityGraph)
   ...
   public void createCoverabilityGraph(RandomAccessFile outputFile,
      ...
      boolean allOmegas;
		...
		LinkedList<Marking>[] state = new LinkedList[root.markup.length];
		for(int i = 0; i < root.markup.length; i++){
			LinkedList<Marking> mlist = new LinkedList<Marking>();
			Marking m = new Marking(dataLayer.getTokenClasses().getFirst(), root.markup[i]);
			mlist.add(m);
			state[i]= mlist;
		}
		
      boolean[] enabledTransitions =
              dataLayer.getTransitionEnabledStatusArray(state);
      ...
      while (!unprocessednodes.isEmpty()) {
		...
		for(int i = 0; i < currentNode.markup.length; i++){
			LinkedList<Marking> mlist = new LinkedList<Marking>();
			Marking m = new Marking(dataLayer.getTokenClasses().getFirst(), currentNode.markup[i]);
			mlist.add(m);
			state[i]= mlist;
		}
