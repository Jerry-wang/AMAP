...
public class GSPN 
    ...
    * @param DataLayer
    */
   private boolean isEFCdGSPN(DataLayerInterface pnmlData) {
      return extendedFreeChoiceNet(pnmlData);
   }


   //Format StateList data nicely.
   protected String renderStateSpaceLinked(DataLayerInterface pnmlData, StateList data) {
      ...
      Place[] places = pnmlData.getPlaces();//pendent++
      for (int i = 0; i < markSize; i++) {
         result.add(places[i].getName());
         //result.add(pnmlData.getPlace(i).getName());
   ...
   //Format StateList data nicely.
   protected String renderStateSpaceLinked(DataLayerInterface pnmlData, StateList data, BufferedWriter writer) {
      if ((data.size() == 0) || (data.get(0).length == 0)) {
         return "n/a";
      }
      int markSize = data.get(0).length;
