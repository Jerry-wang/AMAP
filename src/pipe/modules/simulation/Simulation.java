...
//   private static final String MODULE_NAME = "Simulation";
   ...
   public String simulate(DataLayerInterface data,int cycles,int firings) {
      data.storeState();
      ...
      for (i = 0; i < cycles; i++) {
         ...
         marking = new int[markings.length];
         for(int k = 0; k < markings.length; k++){
       	  marking[k] = markings[k].getFirst().getCurrentMarking();
         }
         if(CreateGui.getApp()!=null) data.restoreState();
         ...
         for (j = 0; j < firings; j++) {
            ...
            Transition fired = data.getRandomTransition();
            if ( fired  == null ) {
